...
def ord(request):
    if request.method == 'POST':
        ...
        for product in products:
            ...
            checkout = Checkout(product=product, customer=Customer(id=customer),
                                address=address, price=product.price,
                                quantity=cart.get(str(product.id)), phone=phone)
            checkout.save()
...
def add_product(request):
    ...
    if request.method == 'POST':
        ...
        if product == '' or category == 'Choose' or price == '':
            ...
            product_det = Product(Category_id=category, name=product,
                                  price=price, description=description,
                                  Color_id=color, img=img)
            product_det.save()
